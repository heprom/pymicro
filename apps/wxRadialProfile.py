...
class ImPanel(PlotPanel):
  ...
  def __init__(self, parent, image=None, color=None, dpi=None, **kwargs):
    ...
    self.max = 65536
    # default controls
    self.update_controls(shape(self.im)[0]/8, shape(self.im)[0]/4, shape(self.im)[0]/2, shape(self.im)[0]/2, -45, 45, 100)
    # initiate plotter
  ...
  def draw(self):
    ...
    self.subplot.plot((self.xc + self.rmin*cos(theta_min), self.xc + self.rmax*cos(theta_min)), \
        (self.yc + self.rmin*sin(theta_min), self.yc + self.rmax*sin(theta_min)), 'y', linewidth=2)
    self.subplot.plot((self.xc + self.rmin*cos(theta_max), self.xc + self.rmax*cos(theta_max)), \
        (self.yc + self.rmin*sin(theta_max), self.yc + self.rmax*sin(theta_max)), 'y', linewidth=2)
    #if self.draw_ref and self.ref_pixels != None:
    if self.draw_ref:
      self.subplot.plot(self.xc + self.ref_pixels*cos(thetas), self.yc + self.ref_pixels*sin(thetas), 'r', linewidth=2)
...
class wxRadialProfileFrame(wx.Frame):
  ...
  def OnSaveProfile(self, event):
    ...
    if self.plotPanel.pix2angle:
      ...
      angles = zeros_like(x)
      for i in range(len(x)):
        angles[i] = atan(tan(self.ref_radius.GetValue()*pi/180.)*x[i]/self.ref_pixels.GetValue())*180./pi
