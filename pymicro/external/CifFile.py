    ...
    * `compat_mode` will allow deprecated behaviour of creating single-dataname loops using
    ...
    def AddCifItem(self, data):
        ...
        if isinstance(data[0], basestring):
            ...
            if isinstance(data[1], (tuple, list)) and not isinstance(data[1],
                                                                     StarFile.StarList):  # a single element loop
...
class CifDic(StarFile.BlockCollection):
    ...
    * do_minimum:
        ...
        for key, value in self.items():
            ...
            if value.has_key("_enumeration_range"):
                ...
                if min == ".":
                    ...
                    self[key].AddLoopItem(
                        (("_item_range.maximum", "_item_range.minimum"), ((max, max, min), (max, min, min))))
        ...
        for parent_block, import_list in import_frames:
            for import_ref in import_list:
                ...
                if full_uri not in self.template_cache:
                    ...
                    self.template_cache[full_uri] = CifDic(dic_as_cif,
                                                           do_minimum=True)  # this will recurse internal imports
    ...
    def create_cat_obj_table(self):
        ...
        base_table = dict([((self[a].get('_name.category_id', '').lower(), self[a].get('_name.object_id', '').lower()),
                            [self[a].get('_definition.id', '')]) \
        ...
        def expand_base_table(parent_cat, child_cats):
            ...
            for child_cat in child_cats:
                ...
                child_names = [(self[n]['_name.object_id'].lower(), self[n]['_definition.id']) for n in
                               self.names_in_cat(child_cat) if self[n].get('_definition.scope', 'Item') == 'Item' and \
                               self[n].get('_type.purpose', '') != 'Key']
                child_names += extra_names
                extra_table.update(dict([((parent_cat, obj), [name]) for obj, name in child_names if
                                         (parent_cat, name) not in extra_table]))
            ...
            child_names += [(self[n]['_name.object_id'].lower(), self[n]['_definition.id']) for n in
                            self.names_in_cat(parent_cat) if self[n].get('_definition.scope', 'Item') == 'Item' and \
                            self[n].get('_type.purpose', '') != 'Key']
            return child_names
    ...
    def names_in_cat(self, cat, names_only=False):
        nameblocks = filter(lambda a: self[a].get("_name.category_id", "").lower()
                                      == cat.lower(), self.keys())
        if not names_only:
            return ["_" + self[a]["_name.category_id"] + "." + self[a]["_name.object_id"] for a in nameblocks if
                    self[a].get('_definition.scope', 'Item') == 'Item']
        else:
    ...
    def change_category_name(self, oldname, newname):
        ...
        if len(oldid) != 1:
            raise CifError, 'Unable to find old category name in category map: %s not in %s' % (
            oldname.lower(), `self.cat_map.items()`)
    ...
    def remove_definition(self, defname):
        ...
        if self[defname].get('_definition.scope') == 'Category':
            child_cats = [a['_name.category_id'] for a in self.get_children(defname) if
                          a.get('_definition.scope') == 'Category']
    ...
    def recurse_child_list(self, parentname):
        ...
        child_blocks = [a for a in self.child_table.keys() if
                        self[a].get('_name.category_id', '').lower() == parentname.lower()]
    ...
    def add_drel_funcs(self):
        ...
        funcnames = [(self[a]["_name.object_id"],
                      getattr(self[a].GetKeyedPacket("_method.purpose", "Evaluation"), "_method.expression")) for a in
                     funclist]
        ...
        for funcname, funcbody in funcnames:
            ...
            py_function = py_from_ast.make_python_function(res_ast, None, targetname=funcname, func_def=True,
                                                           loopable=loop_info, cif_dic=self)
    ...
    def get_final_cats(self, loop_names):
        ...
        try:
            ...
            raise ValidCifError("%s missing from dictionary %s for item in loop containing %s" % (
            self.cat_spec, self.dicname, loop_names[0]))
    ...
    def validate_parent(self, item_name, item_value, whole_block, provisional_items={}, globals={}):
        ...
        if isinstance(item_value, basestring):
            ...
            child_values = item_value[:]  # copy for safety
    ...
    def find_prob_cats(self, whole_block):
        ...
        if len(prob_cats) > 0:
            return (
            False, {'whole_block': [('validate_mandatory_category', {"result": False, "bad_items": problem_cats})]})
        else:
            return (True, {})
    ...
    def run_global_validation(self, item_name, item_value, data_block, provisional_items={}, globals={}):
        results = map(lambda f: (f.__name__, f(item_name, item_value, data_block, provisional_items, globals)),
                      self.global_validation_funs)
...
def merge_dic(diclist, mergemode="replace", ddlspec=None):
    ...
    for dic in diclist:
        ...
        if not isinstance(dic, CifFile):
            dic_as_cif_list.append(CifFile(dic))
        else:
            dic_as_cif_list.append(dic)
